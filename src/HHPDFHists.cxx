...
HHPDFHists::HHPDFHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  for(int i=0; i<100; i++){
    TString name = "max_NN_out_PDF_" + to_string(i+1);
    TString title = "max_NN_out for PDF No. " + to_string(i+1); 
...
void HHPDFHists::fill(const Event & event){
  ...
  //NNoutput.emplace_back(event.get(h_NNoutput3));
  if(NN_classes>4)runtime_error("In HHPDFHists: max. 4 NN categories are implemented ATM");
...
*/
  ...
  if(event.genInfo->systweights().size() > 110 && (!take_ntupleweights)) {
    // this Diboson process has events with >110 and events with <100 systweights :/
    if(!m_oname.Contains("DibosonNLO_ZZ_2L2Nu_2016v3")) throw runtime_error("In HHPDFHists.cxx: Systweights in event.genInfo() is NOT empty but take_ntupleweights is set to 'false'. Is this correct? In this case Thomas says the genInfo weight should be used. Add this sample to take_ntupleweights");
  }

  // Fill histograms
  //cout << "PDFHists: weight: " << weight << endl;
  if(take_ntupleweights) {
    for(int i=0; i<100; i++){
      ...
      //cout << "fillweight: " << fillweight << "for PDF no. " << i << endl;
      hist(name)->Fill(max_NNoutput, fillweight);
    }
  }
  else{
